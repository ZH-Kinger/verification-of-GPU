...
> **注意**：本文件是 H200 时期的散文式标准，对应 `profiles/h200_8gpu.env`。
> B300 / GB300（Blackwell Ultra）按甲方《验收标准》第 1-8 章执行，
> 见 **`docs/acceptance_criteria_b300.md`** 和 `profiles/b300_8gpu.env`，
> 那套标准是逐条数值化、可自动判定的。两者不冲突：本文件的
> "硬性 FAIL 条件"和"判定优先级"仍然适用于所有机型。
