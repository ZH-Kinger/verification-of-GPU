...
标准这一章是"规格核对"，阈值来自合同而不是技术指标，所以做成对照一份采购清单 CSV：

```bash
cp templates/purchase_list_template.csv inventory/purchase_list.csv
vi inventory/purchase_list.csv      # 按合同填；留空的列不参与核对
```

`check_node.sh` 自动读取（也可用 `PURCHASE_LIST=` 指定路径），逐项比对
品牌 / CPU 型号 / CPU 路数 / 内存条数 / 单条容量 / 总容量 / 类型 / 频率 /
NVMe 数量 / 单盘容量 / GPU 型号 / GPU 数量。**不提供清单时各项仍标 `MANUAL`，
绝不会因为没得比就判成通过。**

三个实现要点：

- **SN 列填 `*` 适用于整批，具体 SN 的行优先** —— 同批次里个别机器配置不同时不用另建文件。
- **型号按归一化后互相包含判定**：清单写 `Intel Xeon 6776P`、机器上报
  `Intel(R) Xeon(R) 6776P`，直接比字符串必然不等；忽略大小写、`(R)`/`(TM)`、
  空格与标点后再比。
- **能吃下 Excel 存出来的 CSV**：UTF-8 BOM（不剥掉首列名会变成 `﻿SN`，
  SN 列永远匹配不上）、CRLF 换行（末列字段带 `\r`，数值比对全错）、
  以及型号里含逗号时的引号包裹。这三样任意一个处理不好都会静默匹配失败。

系统盘藏在 RAID 后面看不到物理盘，这一项仍标 `MANUAL`，只做记录。
